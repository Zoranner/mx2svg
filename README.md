...
- **`spacing`**：仅当边 **没有** 显式 `sourcePoint`/`targetPoint`（解析回退为 **源/目标中心连线**）时生效：沿连线穿出形状周界后，两端再各内收 `spacing` 像素。**`ellipse`**（含 **旋转**）：在局部中心坐标系下按轴对齐椭圆求交，与渲染的 `rotate` 一致；**`rhombus` / `hexagon` / `parallelogram` / `triangle` / `trapezoid` / `pentagon`**：与 `shape-path` 一致的 **凸多边形** 边求交（支持 **旋转**）；**`rect` / `internalStorage` 等**：轴对齐框；**旋转矩形**：局部未旋转坐标系内矩形；**`cylinder` / `cloud` / `document` / `dataStorage`** 等曲线轮廓仍用 **外接轴对齐框** 近似。
...
- **近期（高性价比）**：**曲线轮廓**（云、文档等）的 `spacing` 真实周界或更准近似；边标签 **`align`** 等细项。
