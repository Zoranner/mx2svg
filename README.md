...
const svg = convert(drawioXml, {
  ...
  defaultVertexFontSize: 12, // 可选：无 fontSize 的顶点（默认 12）
  defaultEdgeFontSize: 11, // 可选：无 fontSize 的边（默认 11）
});
```
...
常用选项：`--page`、`--padding`、`--bg`、**`--font-stack`**、**`--vertex-font-size`**、**`--edge-font-size`**（与 API 中 **`defaultFontStack`** / **`defaultVertexFontSize`** / **`defaultEdgeFontSize`** 对应）；**`--help`** 查看全部说明。
...
| 多行 | **`</p>` / `</div>` / `<br>` / 换行** 等 → 多 **`tspan`**；垂直居中用 Canvas **`measureText`**（**`mxLabelMultilineVisualCenterDyPx`**），与单行 alphabetic 基线 + 同函数偏移一致 |
