...
- **样式**：**`dashed`** / **`dash=1`**、**`dashPattern`**（同顶点）；**`linecap` / `linejoin`**（折线/曲线；未设时 **`round`/`round`**，与常见连接器一致）；**`opacity`**（整条边 `<g>`，含箭头与标签）；**`strokeOpacity`**（折线/曲线描边；键 **`strokeopacity`**）；**`fillOpacity`**（若有边标签衬底矩形）；**`endArrow` / `startArrow`**（`none`、`open`、`oval`/`dot`、`diamond`、`classic`/`block` 等）；marker 与 **`strokeColor`** 一致；未设 **`startArrow`** 则起点无箭头；**`fontSize` / `fontColor` / `fontStyle` / `fontFamily`** 作用于边标签。
